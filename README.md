...
Requires **Node.js >= 20** to build and install. The package itself runs anywhere `fetch` exists, including Cloudflare Workers with no compatibility flags required — see [Cloudflare Workers](#cloudflare-workers).
...
const client = new DiffbotClient({ token: resolveTokenFromEnv() });
```

`resolveTokenFromEnv()` takes the token from its argument, then an `env`-style object passed as the second argument (`resolveTokenFromEnv(undefined, env)` — the Cloudflare Workers convention), then `$DIFFBOT_API_TOKEN`. Pass a literal string instead if you resolve secrets yourself. Node users who want a `~/.diffbot/credentials` file as a final fallback can import `resolveToken` from `@diffbot/typescript/node` instead — see [Cloudflare Workers](#cloudflare-workers).
...
const client = new DiffbotClient({ token: resolveTokenFromEnv(), timeout: 120_000 });
...
Fetching and caching the ontology is the SDK's job, via an `OntologyStore` — this tool just calls `ontologyStore.load({ refresh })`. By default it builds a plain `OntologyStore`, which caches in memory for the life of the tool instance and dedupes concurrent first calls into one request; pass `refresh: true` on any call to discard the cache and re-fetch. In a Cloudflare Worker, where isolates are recycled between requests, pass a durable store instead so the cache survives across them:

```ts
import { DiffbotClient, KVOntologyStore, resolveTokenFromEnv } from "@diffbot/typescript";
import { DiffbotOntologyTool } from "@diffbot/langchain";

// Inside a Worker's fetch handler, where `env.ONTOLOGY` is a KV namespace binding:
function buildOntologyTool(env: { DIFFBOT_API_TOKEN: string; ONTOLOGY: KVNamespace }) {
  const client = new DiffbotClient({ token: resolveTokenFromEnv(undefined, env) });
  return new DiffbotOntologyTool({
    client,
    ontologyStore: new KVOntologyStore(client, env.ONTOLOGY),
  });
}
```

See [Cloudflare Workers](#cloudflare-workers).
...
const client = new DiffbotClient({ token: resolveTokenFromEnv(), timeout: 60_000 });
...
This package runs in a Cloudflare Worker with **no compatibility flags required**. That's a property of `@diffbot/typescript`, which this package is a thin, unbundled layer over: `@langchain/core` and the SDK's main entry both import no Node builtins, so nothing in the import graph needs `nodejs_compat`.

```ts
import { DiffbotClient, KVOntologyStore, resolveTokenFromEnv } from "@diffbot/typescript";
import { DiffbotKnowledgeGraphTool, DiffbotOntologyTool } from "@diffbot/langchain";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const client = new DiffbotClient({ token: resolveTokenFromEnv(undefined, env) });
    const kg = new DiffbotKnowledgeGraphTool({ client });
    const ontology = new DiffbotOntologyTool({
      client,
      ontologyStore: new KVOntologyStore(client, env.ONTOLOGY),
    });

    const results = await kg.invoke({ query: 'type:Organization name:"Diffbot"' });
    return Response.json(results);
  },
};
```

Two things differ from a Node setup, both because a Worker has no filesystem:

- **Tokens come from `env`, not a credentials file.** `resolveTokenFromEnv(undefined, env)` reads `env.DIFFBOT_API_TOKEN`, then `process.env`. The credentials-file fallback (`resolveToken` from `@diffbot/typescript/node`) isn't importable in a Worker — Node-only code lives behind that subpath specifically so it's never in a Worker's bundle by accident.
- **The ontology cache needs a durable backend.** Worker isolates are recycled between requests, so `DiffbotOntologyTool`'s default in-memory cache (`OntologyStore`) has a poor hit rate there. Pass a `KVNamespace` binding via `ontologyStore: new KVOntologyStore(client, env.ONTOLOGY)` — it satisfies the store's `KVLike` interface directly, with a lock against concurrent refresh stampedes and optional stale-while-revalidate. See `@diffbot/typescript`'s own README for the full `KVOntologyStore` options.

`engines.node >= 20` in `package.json` describes the build/install toolchain, not the runtime — it doesn't gate Workers use.

## Differences from the Python package

Three intentional divergences from `langchain-diffbot`. If you are porting Python code, you will hit all three.
...
### 2. Knowledge Graph and ontology URLs are not overridable
...
### 3. `DiffbotOntologyTool` supports `refresh: true`
...
Admin and utility SDK calls (`crawlListJobs`, `crawlGetJob`, `crawlDeleteJob`) are deliberately not wrapped — they aren't retrievers, loaders, tools, or chat models, and forcing them into one of those shapes would be worse than not having them. Every component exposes its `.client`, so call them directly:

```ts
import { DiffbotClient, crawlListJobs, resolveTokenFromEnv } from "@diffbot/typescript";
import { DiffbotCrawlLoader } from "@diffbot/langchain";

const loader = new DiffbotCrawlLoader({
  client: new DiffbotClient({ token: resolveTokenFromEnv() }),
